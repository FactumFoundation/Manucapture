...
public class G4P_Dialogs extends PApplet {

	/*
	 * Simple sketch to experiment with the different dialogs available to you in
	 * the G4P library.
	 * 
	 * The file diaologs will not work in an applet hosted on a website unless you
	 * digitally sign the jars.
	 * 
	 * The file dialogs match the behaviour of those in Processing V1.5.1 i.e.
	 * immediate mode rather than on a seperate thread as in V2 and V3
	 * 
	...
	public void handleOptionDialog() {
		...
		/*
		 * Some comments on the returned value: G4P.OK and G4P.YES have the same integer
		 * value so can be used interchangeably. G4P.CLOSED maybe returned if the dialog
		 * box is closed although on some systems G4P.NO or G4P.CANCEL may be returned
		 * instead. It is safer to tested for a positive response because they have the
		 * same value. If you must test for a negative response use !G4P.OK or !G4P.YES
		 */
	}
	...
	public void createMessageDialogGUI(int x, int y, int w, int h, int border) {
		...
		String[] t = new String[] { "Plain", "Error", "Information", "Warning", "Question" };
	...
	public void createOptionDialogGUI(int x, int y, int w, int h, int border) {
		...
		t = new String[] { "Yes / No", "Yes / No / Cancel", "Ok / Cancel" };
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "G4P_Dialogs" };
		if (passedArgs != null) {
