...
def capture(camera_id):
    ...
    if debug:
        print ("------------------------------------------------")
        print ("INFO: Capture from camera %i",camera_id)
    ...
    if debug:
        print "Get a list with file names on SD card: DONE!"
    ...
    #print gp.check_result(gp.gp_camera_file_delete(camera, folder, name, context))

    if debug:
        print "Download last image and delete it from SD card: DONE!"
...
#-------------------------------------------------------------------------------
def loop():
    #    1,2,4... el campo orden en el campo import osimport os

    
    while True:
        bytesToRead = serialFromArduino.inWaiting()
        rawData = serialFromArduino.read()
        if rawData == "t":
            if debug:
                print "Capturing!"
            common.system_log.info("-----------------------------------------")             
...
for i in range(0, 9):
    try:
        ...
        if debug:
            print("ERROR: Failed to connect to serial port: %s", serial_port)
