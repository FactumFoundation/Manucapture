...
if isValid :
    ...
    if isReady :
        if bookMeta['source'] == 'dslr':
                items = getCharsAndItems(ORIGPATH, fname)
                nFiles = items['pages']
                nCharts = items['chars']
                print("nItemsA")
                print(items)
                ...
                if nCharts == 0:
                    ...
                    if processColor:
                        if nCharts > 0:
                            ...
                            countFiles(TARGET, "PROFILES", fname, 'process_color', nCharts)
                    ...
                    if processRaw:
                        ...
                        countFiles(TARGET, "TIFF", fname, 'process_raw', nFiles)
                        countFiles(TARGET, "JPEG", fname, 'process_raw', nFiles)
                        countFiles(TARGET, "TIFFP", fname, 'process_raw', nFiles)
            ...
            if processTiff:
                ...
                countFiles(TARGET, "TIFF", fname, 'process_tiff', nFiles)
                countFiles(TARGET, "JPEG", fname, 'process_tiff', nFiles)
                countFiles(TARGET, "TIFFP", fname, 'process_tiff', nFiles)

        if processPDF:
            print("********** create_pdf.py*****************")
            logging.info("********** create_pdf.py *****************")
            subprocess.call(['python', 'create_pdf.py', TARGET, IMTEMP])
            countFiles(TARGET, "PDF", fname, 'create_pdf', 1)
