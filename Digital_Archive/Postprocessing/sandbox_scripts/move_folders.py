...
def verifyDir(PATH1, PATH2):
        ...
        comparison = filecmp.dircmp(PATH1, PATH2, ignore=['Thumbs.db', 'thumbnails', '.@__thumb','project_bp.xml','project_backup.xml'])
...
def moveFiles(PATH1,PATH2):
    ...
    isValid = False
    logging.info("Moving.... {0} TO {1}".format( PATH1,PATH2 ) ) 
    if not os.path.exists(PATH2):
        #os.makedirs(PATH2)
        #copy_tree(PATH1, PATH2)
        copytree(PATH1, PATH2, ignore=ignore_patterns('Thumbs.db', 'thumbnails', '.@__thumb','project_bp.xml','project_backup.xml'))
...
if __name__ == '__main__':
    ...
    try:
        ...
        PATH3 = sys.argv[3] #directorio de loging               
        
        date = time.strftime("%Y%m%d")
        
        LOGFILE = os.path.join(PATH3, "move-report-"+date+".txt" )
        ...
        out = moveFiles(PATH1,PATH2)
        
        if out:
            logging.info("Move project {0} finish without errors".format(project) )
        else:
            logging.error("Move project {0} finish WITH errors".format(project) )
